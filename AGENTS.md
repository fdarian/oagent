...
- `apps/cli/src/index.ts` — Effect CLI with `serve`, `service`, `stdio`, `jobs`, `doctor`, and `claude mcp serve` subcommands. `serve` and `service start` load the embedded SPA filemap from `.gen/web-ui.gen.ts` and delegate to the engine's `createServer`; `--log-file <path>` swaps the default pretty console logger for a JSONL file logger. `service install|restart|status|stop|uninstall` manage the macOS-only launchd LaunchAgent, while `service start` runs the HTTP server directly. `stdio` registers MCP tools over the stdio transport via `registerTools`. Only the commands that need the in-process engine provide `Engine.layer`; the CLI root provides only `BunContext`, deliberately, so `claude mcp serve` (which does NOT need it) never opens the DB and runs orphan-recovery against a live engine's jobs.
- `apps/cli/src/commands/service` — launchd-backed login-item management for macOS. `install` writes `~/Library/LaunchAgents/com.oagent.service.plist` with the compiled binary's `service start` command and loads it with `launchctl bootstrap`; `stop` unloads the running job while retaining the plist; `uninstall` unloads and removes it. `status` reports installation, RunAtLoad, binary, port, and process state. The service requires the built `oagent` binary, not `bun`.
