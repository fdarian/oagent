...
- @effect/platform HttpApi — typed HTTP API (jobs/aliases/models under `/rpc`)
- React 19 + Vite + Tailwind v4 — web SPA, embedded into the binary at build time

## Workspace

- `apps/cli` (`@oagent/cli`) — thin binary entry point.
- `apps/web` (`@oagent/web`) — Vite + React SPA; imports `EngineApi` from `@oagent/engine/api`.
- `services/engine` (`@oagent/engine`) — Effect services (`Jobs`, `OpenCode`), HTTP handlers, Effect HttpApi.
...
- `apps/cli/src/index.ts` — Effect CLI with `serve`, `stdio`, and `claude mcp serve` subcommands. `serve` loads the embedded SPA filemap from `.gen/web-ui.gen.ts` and delegates to `Engine.startServer`. `stdio` registers MCP tools over the stdio transport via `registerTools`. Both build `Engine.layer` in-process; the CLI root provides only `BunContext`, and each command provides `Engine.layer` itself — deliberately, so `claude mcp serve` (which does NOT need it) never opens the DB and runs orphan-recovery against a live engine's jobs.
- `apps/cli/src/commands/claude.ts` + `apps/cli/src/lib/channel.ts` — `oagent claude mcp serve` runs a dedicated [Claude Code channel](https://code.claude.com/docs/en/channels-reference) MCP over stdio. Unlike `stdio`, it does NOT run jobs in-process: it's a thin `HttpApiClient` facade (`engine-client.ts`) bridging to a running engine (`--engine-url`, default `http://localhost:17777` or `$OPENCODE_MCP_PORT`). `start` forks the job and returns `{jobId}` immediately; a fire-and-forget waiter listens on the engine's SSE event stream (`/jobs/:id/events`) until the `__terminal__` sentinel, then fetches the final result once via `jobs.wait` and pushes it into the session as a `notifications/claude/channel` event (`<channel source="oagent" job_id status session_id>…`) — no polling/background-curl needed. The server declares `capabilities.experimental['claude/channel']`; `result`/`cancel` stay as a fallback. Enable with `claude --dangerously-load-development-channels server:<configKey>` during the research preview.
- `services/engine/src/server.ts` — Effect HTTP server via `HttpApiBuilder.serve()` on `BunHttpServer`, with `HttpApiBuilder.Router.use` for custom routes. Typed API at `/rpc/*` (HttpApi groups for jobs/aliases/models); `/mcp` → MCP transport (WebStandard streamable); `/jobs/events` and `/jobs/:id/events` → raw SSE; `/jobs/:id/wait` → long-poll JSON; `*` SPA fallback when `filemap` is provided (404 otherwise). `Engine.startServer({ port, serverInfo, filemap?, portless? })` binds via `BunHttpServer.layer` with EADDRINUSE fallback to port 0. Defaults to port 17777 (overridable via `--port` or `OPENCODE_MCP_PORT`).
- `services/engine/src/cli.ts` — dev-only `@effect/cli` entrypoint for the engine. Exposes a single `serve` subcommand that calls `Engine.startServer` without a filemap. Used by `services/engine/scripts/dev.ts`.
...
- `services/engine/src/http/api.ts` — `EngineApi` HttpApi definition (`jobs`, `aliases`, `models` groups, prefixed `/rpc`). Schemas via `effect/Schema`; exported for `HttpApiClient` inference on clients.
- `services/engine/src/http/api-handlers.ts` — `HttpApiBuilder.group` handlers for each group (`jobsGroupLayer`, `aliasesGroupLayer`, `modelsGroupLayer`), provided into `HttpApiBuilder.api(EngineApi)`.
- `apps/web/src/lib/orpc.ts` — Promise facade over `HttpApiClient.make(EngineApi)` (filename kept for call-site stability). In dev Vite proxies `/rpc` to `ENGINE_URL`; in prod served same-origin from the embedded SPA (or `?engine=` query to override).
