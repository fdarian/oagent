...
By default `apps/cli/scripts/dev.ts` picks the most recently used session, or creates a new one with a random-noun slug on first run.
...
Written by `apps/cli/scripts/dev.ts` on startup; cleaned up on shutdown.
...
| ---------------- | ---------------------------- | ------------------------------------------------------------------------------ |
| `OAGENT_DB_PATH` | `~/.config/oagent/sqlite.db` | SQLite path. `apps/cli/scripts/dev.ts` sets it to the session's `sqlite.db`.  |
| `ENGINE_URL`     | (unset)                      | Used by `apps/web` Vite proxy. `apps/cli/scripts/dev.ts` sets it via dev.json. |
